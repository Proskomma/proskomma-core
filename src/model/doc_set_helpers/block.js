  ...
  const hasMiddleChapter = (b, fromC, toC) => {
    ...
    return (
      blockChapterScopes
        ...
        .filter((n) => n > fromC && n < toC).length > 0
    );
  };
        ...
        (item, openScopes) => {
          ...
          return (
            Array.from(openScopes)
              ...
              .filter((s) => parseInt(s.split('/')[1]) >= fromV).length > 0 ||
            (fromV === 0 &&
              item[0] === 'token' &&
              item[2] &&
              Array.from(openScopes).filter((s) => s.startsWith('verse'))
                .length === 0)
          );
        },
        {}
      ...
      docSet.blockHasMatchingItem(
        ...
        (item, openScopes) => {
          ...
          return (
            Array.from(openScopes)
              .filter((s) => s.startsWith('verse/'))
              .filter((s) => parseInt(s.split('/')[1]) <= toV).length > 0 ||
            (toV === 0 &&
              item[0] === 'token' &&
              item[2] &&
              Array.from(openScopes).filter((s) => s.startsWith('verse'))
  ...
  if (xre.exec(cv, xre('^[1-9][0-9]*$'))) {
    ...
    if (fromV === 0) {
      ...
      return blocks
        ...
        .filter(
          (b) =>
            [...docSet.allBlockScopes(b)].filter((s) => s.startsWith('verse'))
              .length === 0
        );
    } else {
    ...
    const chapterBlocks = blocks.filter((b) =>
      docSet.anyScopeInBlock(b, chapterScopes)
    );
    return chapterBlocks.filter(
      (b) =>
        hasMiddleChapter(b, fromC, toC) ||
        hasFirstChapter(b, fromC, fromV) ||
        hasLastChapter(b, toC, toV)
    );
  } else {
