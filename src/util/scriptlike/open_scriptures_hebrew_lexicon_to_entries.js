...
const itemTokens = (str) =>
  tokenizeString(str).map((tr) => ({
    type: 'token',
    subType: tr[1],
    payload: tr[0],
  }));
...
const rowRecord = (rowArray) => ({
  ...
  items: [
    ...
    ...itemTokens(
      rowArray[4].length > 0 ? rowArray[4].replace(/; $/, '') : rowArray[5]
    ),
    {
      type: 'scope',
      subType: 'end',
...
const lexicon_tsv = fse
  .readFileSync(process ?? process.argv[2])
  .toString()
  .split('\n')
  .map((row) => row.split('\t'))
...
console.log(
  JSON.stringify(
    lexicon_tsv.map((row) => rowRecord(row)),
    null,
    2
  )
);
