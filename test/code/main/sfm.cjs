...
test(`DocSets (${testGroup})`, async function (t) {
  try {
    t.plan(4);
    const query =
      '{ docSets { id lang: selector(id:"lang") abbr: selector(id:"abbr") } }';
...
test(`nDocuments in docSet (${testGroup})`, async function (t) {
  try {
    t.plan(3);
...
test(`DocSet (${testGroup})`, async function (t) {
  try {
    t.plan(6);
...
const [pkPS, pkPSDoc] = pkWithDoc("../test_data/usfm/ps_lat.sfm", {
  lang: "bal",
  abbr: "vid",
});
test(`DocSets (${testGroup})`, async function (t) {
  try {
    ...
    const query =
      '{ docSets { id lang: selector(id:"lang") abbr: selector(id:"abbr") } }';
    const result = await pkPS.gqlQuery(query);
    t.equal(result.errors, undefined);
    t.ok("id" in result.data.docSets[0]);
    t.equal(result.data.docSets[0].lang, "bal");
    t.equal(result.data.docSets[0].abbr, "vid");
  } catch (err) {
    console.log(err);
  }
});
...
test(`DocSet (${testGroup})`, async function (t) {
  try {
    t.plan(6);
    const query = `{ docSet(id: "${pkPSDoc.docSetId}") { id lang: selector(id:"lang") abbr: selector(id:"abbr") documents { id bookCode: header(id: "bookCode") } } }`;
    const result = await pkPS.gqlQuery(query);
    t.equal(result.errors, undefined);
    t.ok("id" in result.data.docSet);
    t.equal(result.data.docSet.documents[0].bookCode, "PS");
    t.equal(result.data.docSet.lang, "bal");
    t.equal(result.data.docSet.abbr, "vid");
    t.ok("id" in result.data.docSet.documents[0]);
  } catch (err) {
    console.log(err);
  }
