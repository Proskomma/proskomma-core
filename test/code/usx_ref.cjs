...
test(
  `Render xt in footnote (${testGroup})`,
  async function (t) {
    try {
      t.plan(3);
      let query =
      ...
      document(id:"${doc.id}") {
        sequence(id:"${seqId}") {
          blocks { items { type subType payload } text(normalizeSpace:true) }
        }
      }
    }`;
...
test(
  ...
  async function (t) {
    try {
      ...
      let result = await pk.gqlQuery(query);
      t.equal(result.errors, undefined);
      const seqId = result.data.documents[0].cv[0].items.filter(i => i.type === 'graft' && i.subType === 'xref')[0].payload;
      ...
      result = await pk.gqlQuery(query);
      t.equal(result.errors, undefined);
      t.ok(result.data.document.sequence.blocks[0].text.includes('1.1'));
    } catch (err) {
      console.log(err);
    }
  },
